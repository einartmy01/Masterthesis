...
This discussion chapter will interpret the result of Chapter~\ref{chap:results} through using the three research questions stated in the introduction. It will also  use the 3GPP TS $\leq$ 100~ms as a value to compare the found results to.
...
For the codec tests the picture is similar. Baseline H.264, passthrough H.264, GPU H.264, passthrough H.265 and GPU H.265 all pass on mean, sitting between 49 and 75~ms. The 720p configurations and baseline CPU H.265 are much closer to the edge, with both sitting above 87~ms and leaving very little margin. The 95 percentile for both goes well past 200~ms. MJPEG at 193~ms mean fails outright and is not a realistic option.
...
The 100~ms figure is a specification target defined at the communication interface level, not a value derived from operator studies in systems like the one tested here. As noted in Subsection 6.6.2, whether a remote train operator can actually drive safely at 88~ms mean latency versus 64~ms is a question the numbers alone cannot answer, and no such study exists for this specific setup. The threshold is a useful reference point, not a hard cutoff with empirically validated consequences.

What the results do show is that several configurations come meaningfully close on the mean. T1 with UDP RTP achieves 54.45~ms, T2 with passthrough achieves 50.40~ms, and C8 with H.265 GPU acceleration achieves 64.80~ms. These are all well beneath 100~ms on the mean, and the spread above that on the 95th percentile is largely driven by occasional network spikes that are characteristic of mobile 5G connections rather than systemic pipeline latency. The configurations that fail the threshold, such as C5 with H.265 at 88.42~ms mean and C9 with MJPEG at 193.14~ms, do so for identifiable reasons that the analysis in this chapter has already addressed.

The more useful frame for these results is not whether they clear 100~ms, but how much of the total glass-to-glass budget they consume. As shown in Subsection 6.1.1, the camera capture and monitor render alone add a worst case of 56.67~ms before the pipeline contributes anything. A pipeline mean of 54~ms on top of that already brings the full glass-to-glass latency close to or above 100~ms regardless of the configuration. Reaching the 3GPP target end-to-end would require not only the best pipeline configuration but also higher frame rate cameras and higher refresh rate monitors, as those hardware choices account for a larger share of the total delay than most codec or protocol decisions do.
\end{comment}
