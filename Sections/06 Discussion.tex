...
The effect of these two added latencies is quite a lot. Resulting at around 50\% of the total G2G latency of the entire system when added. The figures below show how much they add latency to the system.
...
The 3GPP TS 22.289 standard sets a target end-to-end latency of 100~ms for video communication in remote train operation at GoA 3 and GoA 4. Measured against that threshold, none of the configurations in this thesis clear it with margin on the 95th percentile, and several do not meet it even on the mean. It would be easy to read that as a failure, but doing so misrepresents both what the threshold means and what the results actually show.
...
What the results do show is that several configurations come meaningfully close on the mean. T1 with UDP RTP achieves 54.45~ms, T2 with no transcoding achieves 50.40~ms, and C8 with H.265 GPU acceleration achieves 64.80~ms. These are all well beneath 100~ms on the mean, and the spread above that on the 95th percentile is largely driven by occasional network spikes that are characteristic of mobile 5G connections rather than systemic pipeline latency. The configurations that fail the threshold, such as C5 with H.265 at 88.42~ms mean and C9 with MJPEG at 193.14~ms, do so for identifiable reasons that the analysis in this chapter has already addressed.
