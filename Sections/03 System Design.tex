...
For latency measurements, three separated measurements are completed to try to pinpoint where possible issues arise. The system has sender pipeline latency, receiver pipeline latency and transit latency. As mentioned in Probe Placement, both sender and receiver pipeline measurements uses Gstreamer internal pipeline measurements tools, complete avoiding any delay of clock ticks and therefore keeping a very healthy result. Each time video data is passed through the starting probe, it logs the time in a queue list. Then at the last probe it confirms that the queue matches correctly. If there are too many timestamps in the queue, that means a \texttt{leaky=downstream} has gotten rid of an image, and a log of dropped image is recorded. It then compares time now and the time in the queue, calculating the full pipeline measurement.
