...
During testing, the sender and receiver communicate over a local Wi-Fi network provided by a shared access point. Tailscale is installed on both machines and logged in to the same account, providing each machine with a stable 100.x.x.x address. The sender addresses all outgoing RTP streams to the receiver's Tailscale IP, which makes the setup portable: the same scripts and configuration files work regardless of which physical Wi-Fi network the machines are connected to.
...
For the bandwidth-limited test conditions, a software traffic shaper is applied on the sender's network interface to impose an upper bitrate limit, simulating degraded link conditions. This allows the effect of reduced available bandwidth to be evaluated in a controlled and repeatable way without depending on a naturally congested network.

Originally, a plan to use ethernet connection to the receiver computer was in order. However, when running a couple of sprints and iterations, it became apparent that the constraints would not be the Wi-Fi signal at the lab. If this setup is to be test during different conditions, a check of relevant Wi-Fi capabilities is recommended and possible changes as adding direct ethernet connection to routers.
