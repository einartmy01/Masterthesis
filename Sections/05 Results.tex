...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Code/Analyzers/graphs/C1-26.05-09:49/sender_pipeline_26.05-09:49_analysis.png}
    \caption{Sender pipeline latency graph over time C1}\label{fig:C1SenderLatency}
\end{figure}

\textbf{Transit:}

Before the experiments a clock offset error check was completed. For the tests it resulted in a uncertainty of total 7 ms. Which is major effect on the results. Because of this clock offset, all the UDP test resulted in a negative transit latency for the lowest scores. For this reason, the values has been adjusted so the lowest transit latency has been added to all the measurements. Making minimum 0~ms and making max, measured maximum + distance between 0 and measured minimum. The real results for min, and the shift of entire transit measurement was, T1: -4.48, T2: -2.06, T3: -5.26, T5: -2.17. For the codec measurement, it was a higher transit latency so no results was under 0~ms. Even with the clock offset, the transport and codec tests was performed on same offset, which mean that the results are comparable. For a more realistic numbers for transit latency for UDP with RTP see Table~\ref{tab:transit_codec_latency} as all the codec tests used UDP and RTP.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Code/Analyzers/graphs/C1-26.05-09:49/receiver_pipeline_26.05-09:49_analysis.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Code/Analyzers/graphs/C1-26.05-09:49/throughput_26.05-09-49.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Code/Analyzers/graphs/C1-26.05-09:49/sender_cpu_26.05-09:49_analysis.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Code/Analyzers/graphs/C1-26.05-09:49/quality_plot_26.05-09:49.png}
