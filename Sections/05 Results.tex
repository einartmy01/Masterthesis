...
For the unlimited bandwidth, a test of network was performed. The sender computer had a 112 Mbps Upload speed and the receiver had 148~Mbps Download speed. During the poor bandwidth test, a value of 18~Mbps constrain for TCP and 12~Mbps constraint for UDP was used on the Upload speed.

The transport protocol tests examine how the choice of transport layer protocol, encryption, and available bandwidth affect streaming performance. Table~\ref{tab:transport_duration} provides an overview of all six transport test configurations including the duration of test which has direct affect on the package numbers for packet loss calculations. 

\input{Sections/Tables/TransportExpDuration.tex}

The codec configuration tests examine how codec choice, resolution, and encoding hardware affect streaming performance. All tests used UDP - RTP transport with unlimited bandwidth, isolating codec-related variables from transport effects. In Table~\ref{tab:camera_stream_config} is the standard values of the camera encoding used for the \textit{passthrough} tests. Table~\ref{tab:codec_duration} provides an overview of all nine configurations. C1 served as the baseline configuration. As for the duration it gives a slight insight of how fast the car was driving as the test was completed on the same track each time. Worth noting is the fact that duration time is without the first 10 seconds dedicated to startup.

\input{Sections/Tables/CodecExpDuration.tex}
...
The passthrough configurations (T2, C2 and C6) have no sender pipeline stage and are therefore reported as N/A in the sender table. Table~\ref{tab:send_codec_latency} summarises the mean, minimum, maximum and 95\% percentile of latency in the sender pipeline for each codec test.
...
\input{Sections/Tables/Results/T-transit-raw.tex}
...
Throughput was logged at the sender side and reflects the volume of encoded video data leaving the pipeline per second. Under unlimited bandwidth conditions (T1-T4), throughput reflects the natural output of the encoder with a bandwidth of over 100~Mbps. Under the poor bandwidth condition (T5 and T6), throughput is additionally constrained by the available capacity of 18~Mbps and 12~Mbps. The output measured includes the Tailscale wrapping adding a small percentage to the packets size, this is adjusted for in the constraint tests (T5 and T6) by adding a buffer at the constraint.
...
The passthrough configurations T2, C2 and C6 have no encoding step and are therefore reported as N/A.
...
Interpretation of these findings, including their implications for remote train operation and comparison against the 3GPP TS~22.289 latency threshold, is provided in Chapter~\ref{chap:discussion}.
