...
Latency is the central performance parameter in any RTO video system. If the delay between what happens on the track and what the operator sees on screen is too large, safe intervention becomes impossible. The challenge is that no single, universally agreed threshold exists for railway remote operation, and most values currently used in the field are borrowed from research on cars, drones, and cranes~\cite{myhrvold2025}.
...
Empirical studies from railway demonstrations report glass-to-glass (G2G) latency values of 340--380~ms for tramway remote driving using commercial systems~\cite{fp2r2dato2024d41_2}, while the Kozarevic study at NTNU measured an average of around 212~ms using a GPS-synchronised measurement method~\cite{kozarevic2025}. Human factors research from other vehicle domains suggests that operators begin to adjust their behaviour at delays above 250--300~ms, and that latency variability can be more disruptive than a consistently higher but stable delay~\cite{myhrvold2025, neumeier2019}.
...
The \texttt{chronyc tracking} command reports the current estimated offset between the local clock and its reference source, the root mean square offset over recent history, and the estimated residual frequency error. These values give a practical indication of whether the synchronisation quality is sufficient for the measurement being performed. For one-way latency measurements over an internet link, the typical accuracy of a few milliseconds introduces an uncertainty that is small relative to the latency values being measured in streaming systems, but non-negligible when attempting to resolve differences at the single-millisecond level.
% ─────────────────────────────────────────────────────────────────────────────
...
The main difference compared to H.264 is computational cost. Encoding with H.265 requires significantly more processing power, which increases encoding time and places greater demand on the sender hardware~\cite{ohm2012}.
