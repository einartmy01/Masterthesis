...
def build_pipeline():
    ...
    for i, port in enumerate(RTP_PORTS):
        parts.append(
            ...
            f'application/x-rtp-stream, media=video, clock-rate=90000, encoding-name=H264, payload=96 ! '
            f'rtpstreamdepay name=streamdepay{i} ! '
...
# ── rtpstreamdepay probe — one timestamp per frame (real RTP packets) ────────

def make_rtp_probe(cam_idx):
...
def attach_probes(pipeline):
    for i in range(len(RTP_PORTS)):
        # Input stamp: first real RTP packet of each frame (after TCP stream unwrap)
        pipeline.get_by_name(f"streamdepay{i}").get_static_pad("src").add_probe(
