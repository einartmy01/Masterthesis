...
def build_pipeline():
    ...
    for i, port in enumerate(RTP_PORTS):
        parts.append(
            f'udpsrc port={port} '
            f'caps="application/x-rtp, media=video, encoding-name=H265, payload=96" name=src{i} ! '
            f'rtph265depay name=depay{i} ! '
            f'h265parse name=parse{i} ! '
            f'avdec_h265 max-threads=1 name=decoder{i} ! '
            f'tee name=tee{i} '
