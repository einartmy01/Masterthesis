...
H264_PT         = 96
FEC_PT          = 97
STORAGE_SIZE_NS = 220_000_000   # 220 ms (jitter buffer latency + ~20 ms tolerance)
JB_LATENCY_MS   = 200
# ─────────────────────────────────────────────────────────────────────────────
...
def build_pipeline():
    ...
    for i, port in enumerate(RTP_PORTS):
        parts.append(
            f'udpsrc port={port} '
            f'caps="application/x-rtp, payload={H264_PT}, clock-rate=90000" ! '
            f'name=src{i} ! '
            f'rtpstorage size-time={STORAGE_SIZE_NS} name=storage{i} ! '
            f'rtpulpfecdec pt={FEC_PT} name=fecdec{i} ! '
...
def build_pipeline():
    ...
    for i, port in enumerate(RTP_PORTS):
        parts.append(
            f'udpsrc port={port} '
            f'rtpstorage size-time={STORAGE_SIZE_NS} name=storage{i} ! '
            f'rtpssrcdemux name=ssrcdemux{i} '
            f'ssrcdemux{i}. ! '
            f'application/x-rtp, payload={H264_PT}, clock-rate=90000, media=video, encoding-name=H264 ! '
            f'rtpjitterbuffer do-lost=1 latency={JB_LATENCY_MS} ! '
            f'rtpulpfecdec pt={FEC_PT} ! '
            f'rtph264depay ! '
            f'h264parse ! '
...
full_queue    = deque()  # udpsrc src → xvimagesink sink
...
def make_udpsrc_probe(cam_idx):
    ...
    def probe_cb(pad, info):
        ...
        seq, marker, pt = read_rtp_header(buf)
        if seq is None or pt == FEC_PT:
            return Gst.PadProbeReturn.OK
