...
def build_pipeline():
    ...
    for i, port in enumerate(RTP_PORTS):
        srtp_caps = (
            ...
            f"media=(string)video,"
            f"clock-rate=(int)90000,"
            f"encoding-name=(string)H264,"
            f"payload=(int)96,"
            f"ssrc=(uint){SRTP_SSRC[i]},"
            f"srtp-key=(buffer){SRTP_KEY},"
        ...
        parts.append(
            f'udpsrc port={port} caps="{srtp_caps}" name=src{i} ! '
            f'srtpdec name=srtpdec{i} ! '
...
def main():
    ...
    for i in range(len(RTP_PORTS)):
        srtpdec = pipeline.get_by_name(f"srtpdec{i}")
        srtpdec.connect("request-key", lambda *a: print(f"srtpdec{i} request-key fired — SSRC mismatch!"))
