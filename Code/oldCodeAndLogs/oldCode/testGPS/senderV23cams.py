...
def main():
    ...
    # # UDP socket for sending sync packets to receiver
    # sync_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

    # # CSV log
    # csv_file = open(LOG_FILE, "w", newline="")
    # writer = csv.writer(csv_file)
    # writer.writerow(["sender_pts", "gps_time_ns", "stage"])

    # def on_buffer(pad, info, stage):
    #     buf    = info.get_buffer()
    #     gps_ns = vbox.get_gps_time_ns()
    #     pts    = buf.pts if buf.pts != Gst.CLOCK_TIME_NONE else -1

    #     if gps_ns is None or pts == -1:
    #         return Gst.PadProbeReturn.OK

    #     # Log locally
    #     writer.writerow([pts, gps_ns, stage])
    #     csv_file.flush()

    #     # Send sync packet to receiver: 8 bytes pts + 8 bytes gps_time_ns
    #     packet = struct.pack('>QQ', pts, gps_ns)
    ...
    print(f"Sender running. Logging to {LOG_FILE}, sync packets → {RECEIVER_IP}:{SYNC_PORT}")
