...
def analyse(send_path: Path, rec_path: Path, send_suf: str, rec_suf: str) -> dict:
    ...
    # Warn about receiver records with no matching sender entry
    unmatched_rec = [(cam, seq, t) for (cam, seq), t in rec_records.items()
                     if (cam, seq) not in send_records]
    if unmatched_rec:
        section("UNMATCHED RECEIVER RECORDS")
        for cam, seq, t_rec in unmatched_rec:
