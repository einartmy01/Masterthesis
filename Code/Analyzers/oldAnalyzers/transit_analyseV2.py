...
TRANSIT_WARN_MS  = 10.0       # yellow flag above this
TRANSIT_CRIT_MS  = 50.0       # red flag above this
...
def best_pair(send_suffixes: list[str], rec_suffixes: list[str],
    ...
    """
    Among all send/rec suffix pairs within wiggle minutes, return the pair
    whose files are most recently modified. Returns None if no pair found.
    """
    candidates = []
    for ss in send_suffixes:
        for rs in rec_suffixes:
            if minutes_apart(ss, rs) <= wiggle:
                candidates.append((ss, rs))
    if not candidates:
        return None
    # Sort by newest mtime of either file in the pair
    def pair_mtime(pair):
        ss, rs = pair
        return max(
    ...
    return max(candidates, key=pair_mtime)
...
def save_outputs(matched: list, report: dict, log_dir: Path, send_suf: str):
    ...
    base        = log_dir / f"transit_analysis_{send_suf}"
