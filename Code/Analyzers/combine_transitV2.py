...
def load_transit(path: Path, skip_seconds: float) -> dict:
    ...
    for cam, seq, t in raw_rows:
        # Always run normalize_seq — even for skipped rows — so rollover_count
        # stays accurate across the warm-up window.  If a sequence number wraps
        # (65535 → 0) while rows are being discarded we must still account for
        # it, otherwise the first kept rows get the wrong generation offset.
        ...
        if t < cutoff:
            continue  # discard warm-up row, but rollover state is already updated
