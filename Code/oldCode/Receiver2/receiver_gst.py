...
def run(get_time_ns):
    ...
    # Frame counters for tracking frames (one per camera)
    ...
    for i in range(len(RTP_PORTS)):
        ...
        w = csv.writer(f)
        w.writerow(["time_ns", "gst_buffer_pts_ns", "frame_seq", "stage"])
        csv_files.append(f)
        writers.append(w)
    ...
    def make_probe(cam_idx, stage):
        def on_buffer(pad, info):
            ...
            frame_seq = frame_counters[cam_idx]
            writers[cam_idx].writerow([time_ns, pts, frame_seq, stage])
