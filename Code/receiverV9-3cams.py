...
def make_entry_probe(cam_idx):
    """Fires on udpsrc src pad.
    Logs every packet to transit CSV. Only pushes a timing entry for marked
    packets (end of NAL unit) — one timestamp per frame boundary."""
...
def make_exit_probe(cam_idx):
    ...
    Pops the entry timestamp pushed by the matching marked RTP packet and
    computes full receiver pipeline latency. Handles BUFFER and BUFFER_LIST."""
    _mono       = time.monotonic
    _t_in_queue = t_in_queues[cam_idx]
